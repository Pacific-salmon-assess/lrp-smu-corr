...
### How correlated is spawner abundance among Pacific salmon CUs within SMUs?

**B. Connors **

This document briefly describes analyses of correlations in spawner abundance among Pacific salmon Conservation Units within Stock Management Units. 

Data are taken from the [Pacific Salmon Explorer](https://www.salmonexplorer.ca/) and were provided by E. Hertz (Pacific Salmon Foundation). 

All code and associated data to reproduce this document can be found on Github [here](https://github.com/Pacific-salmon-assess/lrp-smu-corr). 


**1. Load required packages and data and wrangle into usable format**
...
**2. Loop over SMUs and calculate average pairwise correlation in spawner abundance among CUs**  
```{r }
...
Plot of within SMU correlations by species. Points are scaled to the number of CUs within a SMU with the largest point equal to 25 CUs.
...
ggplot(smu_cor, aes(x=species, y=avg_corr)) +
  ...
  theme_bw()+
  xlab("Espèce") +
  ylab("Moyenne au sein de la corrélation de l’UGS dans l’abondance des géniteurs") +
    scale_x_discrete(name = "Espèce", 
                      breaks=c("Chinook", "Chum", "Coho", "Pink", "Sockeye"),
                      labels=c("Saumon chinook", 
                               "Saumon kéta", 
                               "Saumon coho",
                               "Saumon rose",
                               "Saumon rouge")) + 
